...
! ***********************************************************************
      ...
      use const_def
      use auto_diff
      use run_star_support, only: mesa_run1_star => run1_star, &
         mesa_start_run1_star => start_run1_star, MESA_INLIST_RESOLVED
     ...
     subroutine py_initialize_mesa(ierr)
         integer, intent(out) :: ierr
         character (len=1000) :: mesa_dir
         integer :: status

         ierr = 0
         call get_environment_variable('MESA_DIR', mesa_dir, status=status)
         if (status /= 0 .or. len_trim(mesa_dir) == 0) then
            mesa_dir = '/opt/mesa-r23.05.1'
         end if

         call const_init(trim(mesa_dir), ierr)
         if (ierr /= 0) return
         call math_init()

      end subroutine py_initialize_mesa


     subroutine py_run1_star_default(ierr)
         integer, intent(out) :: ierr
         logical :: do_alloc_star, do_free_star, okay_to_restart, restart
         integer :: id
         character (len=6) :: inlist_fname
         ...
         call py_initialize_mesa(ierr)
         if (ierr /= 0) return

         MESA_INLIST_RESOLVED = .true.
         call mesa_run1_star( &
            do_alloc_star, do_free_star, okay_to_restart, &
            id, restart, extras_controls, ierr, inlist_fname)
      end subroutine py_run1_star_default

     subroutine py_run1_star( &
            do_alloc_star, do_free_star, okay_to_restart, &
            id, restart, ierr)
         ...
         call mesa_run1_star( &
            do_alloc_star, do_free_star, okay_to_restart, &
            id, restart, extras_controls, ierr, inlist_fname)
      end subroutine py_run1_star

     subroutine py_start_run1_star( &
            do_alloc_star, do_free_star, okay_to_restart, &
            id, restart, restart_filename, pgstar_ok, dbg, ierr)
         logical, intent(in) :: do_alloc_star, do_free_star, okay_to_restart
         integer, intent(inout) :: id
         logical, intent(inout) :: restart
         ...
         if (ierr /= 0) return
         MESA_INLIST_RESOLVED = .true.
         call mesa_start_run1_star( &
            do_alloc_star, do_free_star, okay_to_restart, &
            id, restart, restart_filename, pgstar_ok, dbg, &
            extras_controls, ierr, inlist_fname)
      end subroutine py_start_run1_star
     ...
     subroutine extras_controls(id, ierr)
         ...
         s% other_energy => my_other_energy_routine
 ...
 integer function how_many_extra_profile_columns(id)
    ...
    subroutine my_other_wind_routine(id, Lsurf, Msurf, Rsurf, Tsurf, X, Y, Z, w, ierr)
      ...
      subroutine my_other_energy_routine(id, ierr)
         integer, intent(in) :: id
         integer, intent(out) :: ierr
         type (star_info), pointer :: s
         integer :: k, kernel_id
         real(dp) :: energy_erg, r_inner, r_outer, duration, sigma_frac
         real(dp) :: r_low, r_high, r_mid, width, sigma, radius
         real(dp) :: weight, norm, power, tiny_width

         ierr = 0
         call star_ptr(id, s, ierr)
         if (ierr /= 0) return

         energy_erg = abs(s% x_ctrl(3))
         r_inner = s% x_ctrl(4)
         r_outer = s% x_ctrl(5)
         duration = s% x_ctrl(6)
         sigma_frac = s% x_ctrl(7)
         kernel_id = s% x_integer_ctrl(1)

         if (energy_erg <= 0d0) return
         if (duration <= 0d0) duration = max(s% dt, 1d-99)

         r_low = min(r_inner, r_outer)
         r_high = max(r_inner, r_outer)
         if (r_high <= r_low) return

         tiny_width = max(1d-99, 1d-12*r_high)
         width = max(r_high - r_low, tiny_width)
         r_mid = 0.5d0*(r_low + r_high)
         if (sigma_frac <= 0d0) sigma_frac = 0.2d0
         sigma = max(sigma_frac*width, tiny_width)

         norm = 0d0
         do k = 1, s% nz
            radius = s% r(k)
            if (radius >= r_low .and. radius <= r_high) then
               weight = drag_heat_weight(radius, r_mid, sigma, kernel_id)
               norm = norm + weight*s% dm(k)
            end if
         end do

         if (norm <= 0d0) return

         power = energy_erg/duration
         do k = 1, s% nz
            radius = s% r(k)
            if (radius >= r_low .and. radius <= r_high) then
               weight = drag_heat_weight(radius, r_mid, sigma, kernel_id)
               s% extra_heat(k) = s% extra_heat(k) + power*weight/norm
            end if
         end do

         s% need_to_setvars = .true.
      end subroutine my_other_energy_routine

      real(dp) function drag_heat_weight(radius, r_mid, sigma, kernel_id) result(weight)
         real(dp), intent(in) :: radius, r_mid, sigma
         integer, intent(in) :: kernel_id
         real(dp) :: x

         select case (kernel_id)
         case (1)
            x = (radius - r_mid)/sigma
            weight = exp(-0.5d0*x*x)
         case default
            weight = 1d0
         end select
      end function drag_heat_weight
