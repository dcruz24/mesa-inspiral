...
! ***********************************************************************
      ...
      subroutine do_before_evolve_loop( &
         ...
         call get_environment_variable('MESA_DIR', s% job% mesa_dir)
         if (len_trim(s% job% mesa_dir) == 0) s% job% mesa_dir = '/opt/mesa-r23.05.1'
         s% job% eosDT_cache_dir = './eosDT_cache'
         s% job% kap_cache_dir = './kap_cache'
         s% job% rates_cache_dir = './rates_cache'
         s% job% history_columns_file = 'history_columns.list'
         s% job% profile_columns_file = 'profile_columns.list'
         s% job% chem_isotopes_filename = 'isotopes.data '
         s% job% net_reaction_filename = 'reactions.list'
         s% job% jina_reaclib_filename = ''
         s% job% jina_reaclib_min_T9 = -1d0
         s% job% rate_tables_dir = 'rate_tables'
         s% job% rate_cache_suffix = ''
         s% job% use_suzuki_weak_rates = .false.
         s% job% use_special_weak_rates = .false.
         s% job% special_weak_states_file = 'special_weak_rates.states'
         s% job% special_weak_transitions_file = 'special_weak_rates.transitions'
         s% job% ion_coulomb_corrections = 'none'
         s% job% electron_coulomb_corrections = 'none'
         s% job% color_num_files = 1
         s% job% color_file_names(:) = ''
         s% job% color_file_names(1) = 'lcb98cor.dat'
         s% job% color_num_colors(:) = 0
         s% job% color_num_colors(1) = 6
         s% job% num_special_rate_factors = 0
         s% job% reaction_for_special_factor(:) = ''
         s% job% filename_of_special_rate(:) = ''
         s% job% special_rate_factor(:) = 1d0
         ...
         call get_environment_variable('PYMESA_LOAD_MODEL_FILENAME', temp_fname)
         if (len_trim(temp_fname) > 0) then
            s% job% load_saved_model = .true.
            s% job% load_model_filename = trim(temp_fname)
            s% job% create_initial_model = .false.
         ...
         s% job% T9_weaklib_full_off = 0.01d0
         s% job% T9_weaklib_full_on = 0.02d0
         s% job% weaklib_blend_hi_Z = 26
         s% job% T9_weaklib_full_off_hi_Z = 0.063d0
         s% job% T9_weaklib_full_on_hi_Z = 0.073d0
